...
function pullajax2(){
    $.ajax({
            url: 'http://127.0.0.1:8000/detail/ContainerDetail',
            dataType: 'json',
            type: 'GET',
            success: function(data){
                console.log("success")
                var dmJSON = "http://127.0.0.1:8000/detail/ContainerDetail";
                $.getJSON(dmJSON, function(data) {
                $.each(data.entries, function(i, f) {
                var tblRow = "<tr>" + "<td>" + f.ContainerID + "</td>" + "<td>" + f.PortID + "</td>" + "<td>" + f.PortName + "</td>" +"<td>" + f.PortExportDate + "</td>" + "</tr>"
                $(tblRow).appendTo("#Container-table");
          });
        })
      }})
    }
